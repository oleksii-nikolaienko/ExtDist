...
eWeibull <- function(X,w, method =c("numerical.MLE","moments"),...){
  ...
  if(method == "moments") {
    ...
    WeibullParameters = function(x=NULL, Mean=mean(x), Var=stats::var(x), Second=Var+Mean^2, Output=FALSE){
      ...
      Alpha = stats::uniroot(Func, lower=1, upper=10, Working=Ratio)$root
