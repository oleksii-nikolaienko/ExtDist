...
#' @param logL logical; if TRUE, lBeta.ab gives log likelihood.
#' @param ... other parameters

#' @return dBeta.ab gives the density; pBeta.ab gives the distribution function;
    ...
    
{
  ...
  d <- max(X)-min(X)
  est.par <- wmle(X=X, w=w, distname = "Beta.ab",
...
lBeta.ab <- 
  function(X, w, shape1=2, shape2 =3, a = 0, b = 1,  params = list(shape1, shape2, a, b), logL = TRUE){
    ...
    ll <- sum(w*log(dBeta.ab(x=X,params = params)))
    l <- exp(ll)
