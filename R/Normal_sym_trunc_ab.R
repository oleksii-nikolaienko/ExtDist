...
#' @aliases dNormal_sym_trunc_ab pNormal_sym_trunc_ab qNormal_sym_trunc_ab rNormal_sym_trunc_ab eNormal_sym_trunc_ab lNormal_sym_trunc_ab
...
#' @param logL logical; if TRUE, lNormal_sym_trunc_ab gives log likelihood.
#' @param ... other parameters

#' @return dNormal_sym_trunc_ab gives the density; pNormal_sym_trunc_ab gives the distribution function;
...
dNormal_sym_trunc_ab <-
  function(x, sigma=0.3, a= 0, b=1, params = list(sigma, a, b)){
    if(!missing(params)){
      mu <- params$mu; sigma <- params$sigma; a <- params$a; b <- params$b
    }
    out <- dtrunc( x, spec="norm", a=a, b=b, mean = (a+b)/2, sd = sigma)
    return(out)
  }
...
pNormal_sym_trunc_ab <- 
  function(q, sigma=0.3, a= 0, b=1, params = list(mu=2, sigma = 5, a = 0, b = 1)){
    if(!missing(params)){
      mu <- params$mu; sigma <- params$sigma; a <- params$a; b <- params$b
    }
...
qNormal_sym_trunc_ab <- 
  function(p, sigma=0.3, a= 0, b=1, params = list(mu=2, sigma = 5, a = 0, b = 1)){
    if(!missing(params)){
      mu <- params$mu; sigma <- params$sigma; a <- params$a; b <- params$b
    }
...
rNormal_sym_trunc_ab <- 
  function(n, mu=2, sigma=3, a = 0, b = 1, params = list(sigma, a, b)){
    if(!missing(params)){
      mu <- params$mu; sigma <- params$sigma; a <- params$a; b <- params$b
    }
...
lNormal_sym_trunc_ab <- 
  function(X, w, mu=2, sigma =3, a = 0, b = 1,  params = list(sigma, a, b), logL = TRUE){
    if(!missing(params)){
      mu <- params$mu; sigma <- params$sigma; a <- params$a; b <- params$b
    }
